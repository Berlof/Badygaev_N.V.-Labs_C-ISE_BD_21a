    ...
    public class CoreServiceDB : ICoreService
    {
        ...
        public List<ProductionViewModel> GetList()
        {
            List<ProductionViewModel> result = context.Productions.Select(rec => new ProductionViewModel
            {
                ...
                TimeCreate = SqlFunctions.DateName("dd", rec.TimeCreate) + " " + SqlFunctions.DateName("mm", rec.TimeCreate) + " " + SqlFunctions.DateName("yyyy", rec.TimeCreate),
                TimeImplement = rec.TimeImplement == null ? "" :
            SqlFunctions.DateName("dd",
           rec.TimeImplement.Value) + " " +
        ...
        public void TakeOrderInWork(ProductionBindingModel model)
        {
            using (var transaction = context.Database.BeginTransaction())
            {
                try
                {
                    Production element = context.Productions.FirstOrDefault(rec => rec.Id ==
                   model.Id);
                    if (element == null)
                    {
                    ...
                    {
                        int countOnStores = engineDetail.Number * element.Number;
                        var storeDetails = context.StoreDetails.Where(rec =>
                        rec.DetailId == engineDetail.DetailId);
                        foreach (var storeDetail in storeDetails)
                        {
                            // компонентов на одном слкаде может не хватать
                            if (storeDetail.Number >= countOnStores)
                            {
                                storeDetail.Number -= countOnStores;
                                countOnStores = 0;
                                context.SaveChanges();
                                break;
                            }
                            ...
                            {
                                countOnStores -= storeDetail.Number;
                        ...
                        if (countOnStores > 0)
                        {
                           ...
                           engineDetail.Detail.DetailName + " требуется " + engineDetail.Number + ", не хватает " + countOnStores);
                         }
                    }
        ...
        {
            StoreDetail element = context.StoreDetails.FirstOrDefault(rec =>
