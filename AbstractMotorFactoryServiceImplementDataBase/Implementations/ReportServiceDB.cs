...
namespace AbstractMotorFactoryServiceImplementDataBase.Implementations
{
    public class ReportServiceDB : IReportService
    {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        if (elem.Details.Count() > 0)
                        {
                            ...
                            var excelBorder =
                             ...
                             excelcells.get_Offset(elem.Details.Count()
                            - 1, 1));
                        ...
                        excelcells.Value2 = elem.TotalNumber;
                        excelcells.Font.Bold = true;
                    }
                }
                //сохраняем
        ...
        public List<CustomerProductionsViewModel> GetClientOrders(ReportBindingModel model)
        {
            return context.Productions
                            .Include(rec => rec.Customer)
                            .Include(rec => rec.Engine)
                            .Where(rec => rec.TimeCreate >= model.TimeFrom &&
                                rec.TimeCreate <= model.TimeTo)
                            .Select(rec => new CustomerProductionsViewModel
                            {
        ...
        public void SaveClientOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Paragraph(phraseTitle)
            ...
            doc.Add(paragraph);
            var phrasePeriod = new Phrase("c " + model.TimeFrom.Value.ToShortDateString() +
            " по " + model.TimeTo.Value.ToShortDateString(), new iTextSharp.text.Font(baseFont, 14, iTextSharp.text.Font.BOLD));
           ...
           iTextSharp.text.Font.BOLD);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].TimeCreate, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].EngineName, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].Number.ToString(), fontForCells));
                cell.HorizontalAlignment = Element.ALIGN_RIGHT;
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].Amount.ToString(), fontForCells));
                cell.HorizontalAlignment = Element.ALIGN_RIGHT;
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].State, fontForCells));
                table.AddCell(cell);
            }
            //вставляем итого
            ...
            table.AddCell(cell);
            cell = new PdfPCell(new Phrase(list.Sum(rec => rec.Amount).ToString(), fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_RIGHT,
                Border = 0
            };
